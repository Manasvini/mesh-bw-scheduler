...
ssh cvuser@cv2 -t "bash ~/mesh/tmp/scripts/build_container.sh $1"
ssh cvuser@cv2 -t "sudo ~/mesh/k3s ctr images import ~/mesh/tmp/$1.tar"
ssh cvuser@cv2 -t "sudo chmod +777 ~/mesh/tmp/$1.tar"
...
do
    ...
    ssh cvuser@cv$i -t "sudo ~/mesh/k3s ctr images import ~/mesh/tmp/$1.tar"
